...
Thumbnailer.prototype.execute = function(description, localPaths, onComplete) {
	...
	_.extend(this, {
		localPaths: localPaths,
		extension: localPaths[0].split('.').pop(),
...
Thumbnailer.prototype.videoThumbnail = function () {
	thumbnailCommand = 'ffmpeg  -itsoffset -0  -i ' +  this.localPaths[0] + ' -vcodec mjpeg -vframes 1 -y -filter:v crop=' + this.width + ':' + this.height + ' ' + this.convertedPath;
...
Thumbnailer.prototype.cropped = function() {
  var qualityString = (this.quality ? '-quality ' + this.quality : ''),
    thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" -resize ' + this.width + ' -gravity center -crop ' + this.width + 'x' + this.height + '+0+0 ' + qualityString + ' ' + this.convertedPath;

...
Thumbnailer.prototype.execute = function(description, localPaths, onComplete) {
	...
	this.createConversionPath(function(err) {
		...
		console.log('Mime type >> ' + mime.lookup(_this.extension));
		if (mime.lookup(_this.extension).indexOf('video') === 0) {
			console.log('Calling video thumbnail function...');
...
Thumbnailer.prototype.videoThumbnail = function () {
	thumbnailCommand = 'ffmpeg  -itsoffset -1  -i ' +  this.localPath + ' -vcodec mjpeg -vframes 1 -an -f rawvideo -s ' +  (this.width + 100) + 'x' + (this.height + 100) + ' ' + this.convertedPath;
...
/**
 * Convert the image using the cropped strategy
 */
Thumbnailer.prototype.cropped = function() {
  var qualityString = (this.quality ? '-quality ' + this.quality : ''),
    thumbnailCommand = config.get('convertCommand') + ' "' + this.localPath + '[0]" -resize ' + (this.width + 100) + 'x' + (this.height + 100) + '^ -gravity center -crop ' + this.width + 'x' + this.height + '+0+0 ' + qualityString + ' ' + this.convertedPath;
